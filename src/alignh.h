...
class AlignHirsch {
 public:
  AlignHirsch(const char *q, int64_t ql, const char *t, int64_t tl, Penalties p,
        AlignType aln_type, GlobalMargins gm);
  AlignHirsch(const std::string &q, const std::string &t, Penalties p,
        AlignType aln_type, GlobalMargins gm);
  ~AlignHirsch();
 ...
 private:
  AlignHirsch(const AlignHirsch& op) = delete;
  AlignHirsch& operator=(const AlignHirsch& op) = delete;
