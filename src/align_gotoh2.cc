...
AlignGotoh2::AlignGotoh2(const std::string& q, const std::string& t, Penalties p,
             AlignType aln_type, GlobalMargins gm)
    : AlignGotoh2(q.c_str(), q.size(), t.c_str(), t.size(), p, aln_type, gm) {
}

AlignGotoh2::AlignGotoh2(const char* q, int64_t ql, const char* t, int64_t tl, Penalties p,
             AlignType aln_type, GlobalMargins gm)
              : Align(q, ql, t, tl, p, aln_type, gm) {
  Align_(q, ql, t, tl, p, gm, aln_type, q_start_, q_end_, t_start_, t_end_, score_, cigar_);
}

AlignGotoh2::~AlignGotoh2() {

}

int AlignGotoh2::Align_(const char* q, int64_t ql, const char* t, int64_t tl, Penalties p, GlobalMargins gm, AlignType aln_type,
...
int AlignGotoh2::AlignGlobal_(const char* q, int64_t ql, const char* t, int64_t tl, Penalties p, GlobalMargins gm,
                       int32_t &q_start, int32_t &q_end, int32_t &t_start, int32_t &t_end, int32_t &score, std::vector<is::CigarOp> &cigar) {
...
int AlignGotoh2::AlignLocal_(const char* q, int64_t ql, const char* t, int64_t tl,
                      Penalties p) {
  return 0;
}
